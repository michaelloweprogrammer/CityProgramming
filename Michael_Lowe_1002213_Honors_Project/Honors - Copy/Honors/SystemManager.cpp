...
bool SystemManager::Initialize()
{
	...
	{
		m_Sound[i] = new Audio;
		if(!m_Sound[i]){return false;}
	}
	// Initialize the sound object.
	m_Sound[0]->setAudio("../Honors/Resources/Audio/sound02.wav"); // 3d sound
	result = m_Sound[0]->Initialize(m_hwnd,1);
	...
	for (int i = 0; i <100; i++)
	{
		m_Sound2[i] = new twoDAudio;
		if(!m_Sound2[i]){return false;}
	}
	// Initialize the sound object.
	
	result = m_Sound2[0]->Initialize(m_hwnd);
	if(!result){MessageBox(m_hwnd, L"Could not initialize Direct Sound.", L"Error", MB_OK);return false;}
...
LRESULT CALLBACK SystemManager::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	switch(umsg)
	{
		...
		case WM_KEYUP:
		{
			// If a key is released then send it to the input object so it can unsent the state for that key.
			m_Input->KeyIsUp((unsigned int)wparam);
			return 0;
		}
...
void SystemManager::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	m_applicationName = L"HONORS PROJECT";
