...
bool Audio::setAudio(char* Location)
{
	location = Location;
	return 0;
}
bool Audio::Initialize(HWND hwnd,int type)
{
	bool result;
	int  audiotype = type;
	...
	result = LoadWaveFile(location, &m_secondaryBuffer1, &m_secondary3DBuffer1);
...
bool Audio::InitializeDirectSound(HWND hwnd)
{
	HRESULT result;
...
bool Audio::LoadWaveFile(char* filename, IDirectSoundBuffer8** secondaryBuffer, IDirectSound3DBuffer8** secondary3DBuffer)
{
	int error;
	FILE* filePtr;
	unsigned int count;
...
void Audio::ShutdownWaveFile(IDirectSoundBuffer8** secondaryBuffer, IDirectSound3DBuffer8** secondary3DBuffer)
{
